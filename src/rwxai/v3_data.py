...
def load_cicddos2019(data_dir: str | Path, smoke: bool = False) -> RawDataset:
    """Load the prepared CIC-DDoS2019 cross-day sample.

    The raw capture spans roughly 70 million flows, so the cache built by
    ``prepare_cicddos2019_cache.py`` is used instead of re-parsing 30 GB per
    seed. Day 01-12 is the training period and day 03-11 the later held-out day,
    mirroring the CIC-IDS-2017 cross-day protocol. Attack traffic is the
    majority class here, which inverts the imbalance direction of the other
    three datasets.
    """
    path = Path(data_dir)
    if path.is_dir():
        path = path / "cicddos2019.npz"
    cache = np.load(path, allow_pickle=True)
    columns = [str(name) for name in cache["columns"]]
    X_train = pd.DataFrame(cache["X_train"], columns=columns)
    y_train = cache["y_train"].astype(np.int64)
    X_test = pd.DataFrame(cache["X_test"], columns=columns)
    ...
    if smoke:
        X_train, y_train = _stratified_cap(X_train, y_train, 400, CIC_CAP_SEED)
        X_test, y_test = _stratified_cap(X_test, y_test, 250, CIC_CAP_SEED + 1)

    train_audit = audit.get("train", {})
    test_audit = audit.get("test", {})
    return RawDataset(
        X_train=X_train,
        y_train=y_train,
        X_test=X_test,
        ...
        protocol={
            "partition_type": "cross-day temporal holdout",
            "temporal_holdout": True,
            "training_period": "capture day 2018-11-03 (directory CSV-03-11)",
            "test_period": "capture day 2018-12-01 (directory CSV-01-12)",
            "capture_dates_verified_from": "Timestamp field of the raw CSV files",
            "test_partition_is_sampled": True,
            "full_training_period_rows": train_audit.get("full_period_rows"),
            "full_test_period_rows": test_audit.get("full_period_rows"),
            "full_training_attack_rate": train_audit.get("full_attack_rate"),
            "full_test_attack_rate": test_audit.get("full_attack_rate"),
            "sampled_training_rows": train_audit.get("sampled_rows"),
            "sampled_test_rows": test_audit.get("sampled_rows"),
            "training_cap_seed": audit.get("cap_seed", CIC_CAP_SEED),
            "training_cap_policy": (
                "fixed stratified sample preserving binary prevalence; both "
                "capture days are sampled because the full release is ~70 "
                "million flows"
            ),
            "training_attack_labels": train_audit.get("attack_labels"),
            "test_attack_labels": test_audit.get("attack_labels"),
            "test_only_attack_labels": audit.get("test_only_attack_labels"),
            "dropped_identity_columns": audit.get("dropped_identity_columns"),
            "binary_task": "BENIGN versus ATTACK",
            "imbalance_direction": "attack majority",
        },
    )
